...
class SmsProxy {
    ...
    getDestinationRealNumber(from) {
        ...
        const fromUserA = (parseInt(from) === parseInt(this.chat.userA));
        const fromUserB = (parseInt(from) === parseInt(this.chat.userB));
    ...
    proxySms(from, text) {
        ...
        if (destinationRealNumber  === null) {
            console.log(`No chat found for this number ${from}`);
            return;
        }
